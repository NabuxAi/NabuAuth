// Package mcp serves the Model Context Protocol over NabuAuth's own HTTP
...
// one with Failf) becomes an isError result; anything else is logged and
// replaced with a fixed string, because an arbitrary error from a provider
// client or a database driver is exactly where a credential leaks.
type Tool struct {
	Name        string
	Description string
